...
| 5.2 | `make check` tests output equivalence, not heap allocation; there is no `operator new` poison test. | **Fixed.** `tests/test_no_heap.cpp` overrides global `operator new`/`delete` and verifies zero heap allocations from framework primitives (Graph, HSM, EventGraph, storage) in static mode. Runs as part of `make test`. |
| 5.3 | `inference.hpp` used global `Config` rather than graph `Cfg`. | **Fixed** (§2.1). |
| 5.4 | `LlamaCppEngine` is heap-based and not gated by `StaticAlloc`. | **Fixed.** `LlamaCppEngine` is disabled when `EMBG_STATIC_ALLOC` is defined; a `static_assert` fires with a clear message directing users to `StubEngine` or a custom heap-free engine. |
